...
function AdminTable() {
    ...
    const fetchAdminData = () => {
      axios.get('http://localhost:8000/users')
        .then(response => {
          ...
          const adminData = response.data.filter(user => user.role === 'admin');
  ...
  const handleAddOrUpdateAdmin = () => {
    ...
    if (modalMode === 'edit') {
      axios.put(`http://localhost:8000/users/${currentAdmin.id}`, adminPayload)
        .then(() => {
      ...
      const existingAdmin = adminData.find(admin => admin.companyName === companyName);
      ...
      axios.post('http://localhost:8000/users', adminPayload)
        .then(() => {
